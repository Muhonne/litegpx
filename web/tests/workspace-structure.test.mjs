...
const featureDocs = [
  ...
  "mobile-app-settings.md",
  "data-service-route-corridor-generation.md",
...
function isGeneratedArtifactRef(ref) {
  return ref.startsWith("shared/maps/") ||
    ref.startsWith("mapdataservice/output/") ||
    ref.startsWith("mapdataservice/cache/");
}
...
async function assertWorkspaceCodeRefsExist(path) {
  ...
  const codeRefs = [...text.matchAll(/`((?:AGENTS\.md|docs|web|mobile|mapdataservice|shared)\/?[^`\s]*)`/g)]
    .map((match) => match[1])
    .filter((ref) => ref && !ref.includes("*") && !isGeneratedArtifactRef(ref));
...
const mobileReadme = await readFile(join(workspace, "mobile/README.md"), "utf8");
...
assert.match(featuresIndex, /docs\/archive/, "FEATURES.md should identify where superseded narrative docs live");
...
for (const featureDoc of featureDocs) {
  ...
  const codeRefs = [...text.matchAll(/`((?:web|mobile|mapdataservice|shared)\/[^`]+)`/g)]
    ...
    .filter((ref) => !isGeneratedArtifactRef(ref));
...
for (const docPath of [
  ...
  "docs/FEATURES.md",
  "docs/USE_CASES.md",
  "docs/archive/mobile-iteration-log.md",
  ...featureDocs.map((featureDoc) => `docs/features/${featureDoc}`),
]) {
  await assertWorkspaceCodeRefsExist(docPath);
}

const corridorDoc = await readWorkspaceFile("docs/features/data-service-route-corridor-generation.md");
for (const expected of [
  ...
  assert.match(corridorDoc, expected, "corridor feature doc should cover generated corridor artifacts");
}

const mobileSettingsDoc = await readWorkspaceFile("docs/features/mobile-app-settings.md");
for (const setting of [
  ...
  assert.ok(
    mobileSettingsDoc.includes(setting) && mobileReadme.includes(setting),
    `mobile settings docs should mention ${setting}`,
  );
}
