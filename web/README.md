...
`10-mobile-save-button.sh` covers save-to-mobile request payloads, busy button feedback while map data is being generated, immediate route-manager update after save, clearing an excluding route filter, and preserving the saved route and metadata across stale catalog refreshes.
`11-mobile-route-management.sh` covers filtering bundled mobile routes, exposing all filtered routes in the visible list, selecting a visible route-list item with mouse or arrow keys, marking the loaded and unsaved route states, loading a selected route with Enter or the load button, preserving mobile route identity on save, showing saved/unsaved mobile edit state, and cancelling a load without moving the picker away from the loaded route.
