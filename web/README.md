...
`11-mobile-route-management.sh` covers filtering bundled mobile routes, exposing all filtered routes in the visible list, selecting a visible route-list item, loading a selected route with Enter or the load button, preserving mobile route identity on save, showing saved/unsaved mobile edit state, and cancelling a load without moving the picker away from the loaded route.
