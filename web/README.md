...
`11-mobile-route-management.sh` covers filtering bundled mobile routes and dirty draft route names, restoring loaded-route selection after no-match filters, exposing all filtered routes in the visible list, selecting a visible route-list item with mouse or arrow keys, marking the loaded and unsaved route states, showing the draft name for unsaved loaded routes, disabling the clean loaded route action, exposing and completing a revert action for dirty loaded routes, loading a selected route with Enter or the load button, preserving mobile route identity/title on save and refresh, keeping the saved route visible during refresh, showing saved/unsaved mobile edit state, and cancelling a load without moving the picker away from the loaded route.
