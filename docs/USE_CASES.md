...
Feature: Web GPX route creation
  ...
  Scenario: Undo and redo route drawing mistakes
    ...
    And duplicate endpoint clicks or adjacent inserts do not add a route point or consume undo history
...
Feature: Web GPX import and editing
  ...
  Scenario: Load a bundled mobile route for editing
    ...
    And double-clicking the clean loaded route does not reload or refit it
    ...
    And the route filter can find the unsaved loaded route by that draft route name
