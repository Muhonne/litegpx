...
The web app lets the user draw a GPX route on desktop, refine the points, inspect distance and point count, and export a mobile-compatible GPX file.
...
Feature: Web GPX route creation
  Scenario: Draw and export a new route
    ...
    Then the route is shown in edit mode with updated distance and point count
